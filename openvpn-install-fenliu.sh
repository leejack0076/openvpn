...
echo "route-up /etc/openvpn/client-route-up.sh" >> ~/"$client".ovpn
...
echo 'route-up /etc/openvpn/route-up.sh' >> /etc/openvpn/server/server.conf
...
verb 3" > /etc/openvpn/server/client-common.txt
	...
	# 创建update-resolv-conf脚本
cat << 'EOF' > /etc/openvpn/update-resolv-conf
#!/bin/bash
[ -n "$script_type" ] && [ "$script_type" = "down" ] && exit 0
[ -n "$local_ip" ] && [ -n "$remote_ip" ] && {
    local_network=$(ip -4 route get $local_ip | awk '{print $3}')
    if [ "$local_network" = "0.0.0.0" ]; then
        local_network="127.0.0.1"
    fi
    ip route add $local_network/32 dev lo
}
EOF
...
# 添加示例路由
ip route add 192.168.0.0/16 via 10.8.0.1
# 根据需要添加其他路由
EOF
...
# 添加国内IP段直连规则（示例）
ip route add 1.0.0.0/8 dev $dev
# 继续添加其他国内IP段规则
EOF
